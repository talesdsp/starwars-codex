...
describe("Codex reducer", () => {
  test("Case default return initial value", () => {
    const state = codexReducer(undefined, {
      type: "default",
  ...
  test("Case IS_LOADING return isLoading as truthy", () => {
    const state = codexReducer(initialState, triggerLoading())
  ...
  test("Case SET_DATA return input data and isLoading as falsy", () => {
    const state = codexReducer(initialState, setData(fetchedData))
    expect(state).toEqual({
      data: state.data,
      isLoading: false,
    })
...
describe("getData()*", () => {
  describe("assert logic", () => {
    describe("fetch data, update reducer, finish", () => {
    ...
    describe("fail request, trigger catch error, finish", () => {
      testSaga(getData, action)
        .next()
        .call(swapi, action.payload.urlPath)
        .throw(new Error())
  ...
  describe("assert proper execution", () => {
    ...
    it("catch error for undefined payload", () => {
      ...
      expect(dispatchedActions[0]).toEqual(
        failedRequest(
          new TypeError("Cannot read property 'payload' of undefined"),
        ),
      )
    })
    ...
    it("catch error for undefined path", () => {
      ...
      expect(dispatchedActions[1]).toEqual(
        failedRequest(
          new TypeError("Cannot read property 'urlPath' of undefined"),
        ),
      )
    })
